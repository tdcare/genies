...
pub(crate) fn impl_wrapper(target_fn: &ItemFn, _args: &AttributeArgs) -> TokenStream {
    ...
    if !is_async {
        panic!(
            "[ddd_dapr] #[wrapper] 'fn {}({})' must be  async fn! ",
    ...
    let wrapper_feignhttp_code = quote! {
        pub async fn #func_name_ident(#func_args) -> #return_ty{
             let bearer = ddd_dapr::config::REMOTE_TOKEN.lock().unwrap().access_token.clone();
        ...
        return if feignhttp_return.is_ok() {
            ...
            if err.to_string().contains("401 Unauthorized") {
                let remote_token_new = ddd_dapr::auth::jwt::get_temp_access_token(
                    &ddd_dapr::config::CONTEXT.config.keycloak_auth_server_url,
                    &ddd_dapr::config::CONTEXT.config.keycloak_realm,
                    &ddd_dapr::config::CONTEXT.config.keycloak_resource,
                    &ddd_dapr::config::CONTEXT.config.keycloak_credentials_secret,
                ).await;
                ddd_dapr::config::REMOTE_TOKEN.lock().unwrap().access_token = remote_token_new.clone();
