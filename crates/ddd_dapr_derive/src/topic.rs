...
pub(crate) fn impl_topic(target_fn: &ItemFn, args: &AttributeArgs) -> TokenStream {
    ...
    let salvo_code=quote!{
         ...
         pub async fn #m_salvo_wrap(_req: &mut Request, _depot: &mut Depot, _res: &mut Response){
            ...
            let CONSUME_STATUS_CONSUMING: &str = "CONSUMING";
            let CONSUME_STATUS_CONSUMED: &str = "CONSUMED";
            ...
            // let cloud_event: CloudEvent = serde_json::from_slice(&body).unwrap_or_default();
                ...
                match v {
                    ...
                    _=> {
                        ...
                        if set_CONSUMING.is_ok() {
                            log::debug!("设置事件为正在消费中，开始调用事件处理程序");
