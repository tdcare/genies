...
pub(crate) fn impl_wrapper(target_fn: &ItemFn, _args: &AttributeArgs) -> TokenStream {
    ...
    let wrapper_feignhttp_code = quote! {
        pub async fn #func_name_ident(#func_args) -> #return_ty{
             let bearer = genies_context::REMOTE_TOKEN.lock().unwrap().access_token.clone();
        ...
        return if feignhttp_return.is_ok() {
            ...
            if err.to_string().contains("401 Unauthorized") {
                let remote_token_new = genies_core::jwt::get_temp_access_token(
                    &genies_context::CONTEXT.config.keycloak_auth_server_url,
                    &genies_context::CONTEXT.config.keycloak_realm,
                    &genies_context::CONTEXT.config.keycloak_resource,
                    &genies_context::CONTEXT.config.keycloak_credentials_secret,
                ).await;
                genies_context::REMOTE_TOKEN.lock().unwrap().access_token = remote_token_new.clone();
