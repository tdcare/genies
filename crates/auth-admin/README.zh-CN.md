...
- **OAuth 2.0 授权服务器** — RFC 6749 标准实现，支持 PKCE
...
- **OAuth 2.0 授权服务器**：RFC 6749 标准实现，支持 authorization_code（含 PKCE）、password、client_credentials、refresh_token 四种授权模式
...
│   │   ├── oauth_handler.rs       #   OAuth 2.0 端点（授权/令牌/内省/撤销）
│   │   ├── oauth_client_handler.rs #  OAuth 客户端 CRUD 管理
│   │   └── app_proxy_handler.rs   # 多应用 API 代理
│   └── dto/                   #   请求/响应 DTO
├── application/               # 应用层
│   ├── service.rs             #   AuthService、UserService、RoleService 等
│   ├── app_service.rs         #   ApplicationAppService
│   ├── oauth_token_service.rs #   OAuthTokenService（授权/令牌/内省/撤销）
│   ├── oauth_client_service.rs #  OAuthClientAppService（客户端 CRUD）
│   ├── oauth_dto.rs           #   OAuth DTO
...
│   │                          #     OAuthDomainService
│   ├── repository/            #   RBatis 仓储实现（含 oauth_*_repository）
...
公开路由（登录、登出、刷新 Token、OAuth 授权/令牌/内省/撤销、Admin UI）跳过认证。
...
| `/auth-admin/oauth/authorize` | GET | OAuth 2.0 授权端点 |
| `/auth-admin/oauth/token` | POST | OAuth 2.0 令牌端点 |
| `/auth-admin/oauth/introspect` | POST | OAuth 2.0 令牌内省 |
| `/auth-admin/oauth/revoke` | POST | OAuth 2.0 令牌撤销 |
...
auth-admin 提供完整的 RFC 6749 OAuth 2.0 授权服务器。令牌端点使用 `application/x-www-form-urlencoded` 格式。

**支持的授权模式：**

| 授权模式 | 说明 | 需要 client_secret | 需要用户认证 |
|----------|------|-------------------|-------------|
| `authorization_code` | 授权码模式（支持 PKCE） | 否（仅 PKCE 验证） | 是（通过 `/oauth/authorize`） |
| `password` | 资源所有者密码凭证模式 | 是 | 是（用户名+密码） |
| `client_credentials` | 客户端凭证模式（机器对机器） | 是 | 否 |
| `refresh_token` | 刷新令牌模式 | 是 | 否 |
...
1. GET /oauth/authorize?response_type=code&client_id=xxx&redirect_uri=xxx
   ...
   → 302 重定向: {redirect_uri}?code=<授权码>&state=xxx

2. POST /oauth/token
   grant_type=authorization_code&code=<授权码>&client_id=xxx&redirect_uri=xxx
   → { access_token, token_type, expires_in, refresh_token, scope }

3. POST /oauth/token
   grant_type=refresh_token&refresh_token=<刷新令牌>&client_id=xxx&client_secret=<密钥>
   → { access_token, token_type, expires_in, refresh_token (已轮换), scope }

4. POST /oauth/introspect
   token=<访问令牌>  →  { active: true, scope, client_id, username, sub, exp, iat }

5. POST /oauth/revoke
   token=<令牌>&token_type_hint=refresh_token  →  200 OK（空响应）
```

- 授权端点从 `Authorization: Bearer <JWT>` 请求头读取用户身份
- 授权码有效期为 10 分钟，且仅能使用一次
- 刷新令牌支持轮换（每次刷新时旧令牌被撤销）
- authorization_code 模式支持 PKCE（S256）

#### OAuth 客户端管理

| 端点 | 方法 | 功能 |
|------|------|------|
| `/auth-admin/oauth/clients` | GET | 客户端列表（分页） |
| `/auth-admin/oauth/clients` | POST | 创建 OAuth 客户端 |
| `/auth-admin/oauth/clients/{id}` | GET | 客户端详情 |
| `/auth-admin/oauth/clients/{id}` | PUT | 更新客户端 |
| `/auth-admin/oauth/clients/{id}` | DELETE | 删除客户端 |
| `/auth-admin/oauth/clients/{id}/regenerate-secret` | POST | 重新生成客户端密钥 |
...
| `auth_oauth_clients` | OAuth 2.0 客户端注册 |
| `auth_oauth_authorization_codes` | 临时授权码（10分钟有效期） |
| `auth_oauth_access_tokens` | OAuth 访问令牌（仅 opaque 模式） |
| `auth_oauth_refresh_tokens` | OAuth 刷新令牌（支持轮换） |
| `message` | Dapr 消息发件箱 |
