...
- **OAuth 2.0 Authorization Server** — RFC 6749 compliant, supporting authorization_code, password, client_credentials, and refresh_token grant types with PKCE
...
│   ├── oauth_dto.rs           # OAuth DTOs (authorize/token/introspect/revoke requests & responses)
...
│   │                    #   OAuthClientEntity, OAuthAuthorizationCodeEntity,
...
│   │   ├── oauth_client_entity.rs # OAuthClientEntity (auth_oauth_clients)
│   │   ├── oauth_authorization_code_entity.rs # OAuthAuthorizationCodeEntity
│   │   ├── oauth_access_token_entity.rs # OAuthAccessTokenEntity
│   │   └── oauth_refresh_token_entity.rs # OAuthRefreshTokenEntity
...
│   │   ├── oauth_client_repository.rs  # OAuth client queries
│   │   ├── oauth_authorization_code_repository.rs # Auth code queries
│   │   ├── oauth_access_token_repository.rs # Access token queries
│   │   └── oauth_refresh_token_repository.rs # Refresh token queries
...
│       ├── oauth_domain_service.rs # OAuthDomainService — client CRUD, auth codes, tokens
...
Three route groups:
1. **Public** — `/auth-admin/login`, `/auth-admin/logout`, `/auth-admin/refresh`, `/auth-admin/captcha`, `/auth-admin/2fa/verify`, `/auth-admin/oauth/authorize`, `/auth-admin/oauth/token`, `/auth-admin/oauth/introspect`, `/auth-admin/oauth/revoke`, `/auth-admin/ui/**`
...
auth-admin provides a full RFC 6749 compliant OAuth 2.0 Authorization Server. All OAuth endpoints are public (no JWT required for token exchange — the client secret is used for authentication).

| Endpoint | Method | Auth | Description |
|----------|--------|------|-------------|
| `/auth-admin/oauth/authorize` | GET | Public | Authorization endpoint (redirects with `code`) |
| `/auth-admin/oauth/token` | POST | Public | Token endpoint (4 grant types) |
| `/auth-admin/oauth/introspect` | POST | Public | Token introspection (RFC 7662) |
| `/auth-admin/oauth/revoke` | POST | Public | Token revocation (RFC 7009) |

#### Supported Grant Types

| Grant Type | Description | Requires client_secret | Requires user auth |
|------------|-------------|----------------------|--------------------|
| `authorization_code` | Authorization Code grant (with PKCE) | No (PKCE verification only) | Yes (via `/oauth/authorize`) |
...
```
1. GET /auth-admin/oauth/authorize?response_type=code&client_id=admin-ui
   &redirect_uri=http://localhost:9099/ui/&scope=openid+profile&state=xyz
   Authorization: Bearer <JWT_TOKEN>

   → 302 redirect: {redirect_uri}?code=<AUTH_CODE>&state=xyz

2. POST /auth-admin/oauth/token
   Content-Type: application/x-www-form-urlencoded
   grant_type=authorization_code&code=<AUTH_CODE>&client_id=admin-ui
   &redirect_uri=http://localhost:9099/ui/

   → { access_token, token_type, expires_in, refresh_token, scope }

3. POST /auth-admin/oauth/token  (refresh)
   Content-Type: application/x-www-form-urlencoded
   grant_type=refresh_token&refresh_token=<RT>&client_id=admin-ui
   &client_secret=<SECRET>

   → { access_token, token_type, expires_in, refresh_token (new), scope }

4. POST /auth-admin/oauth/introspect
   Content-Type: application/x-www-form-urlencoded
   token=<ACCESS_TOKEN>

   → { active: true, scope, client_id, username, sub, exp, iat, token_type }

5. POST /auth-admin/oauth/revoke
   Content-Type: application/x-www-form-urlencoded
...
**Key notes:**
- The authorization endpoint reads user identity from the `Authorization: Bearer <JWT>` header (NOT from cookies)
- Token endpoint uses `application/x-www-form-urlencoded` format (NOT JSON)
- `refresh_token` grant requires `client_secret` (bcrypt verified)
- Refresh tokens are rotated: each refresh revokes the old refresh token and issues a new one
- Authorization codes expire in 10 minutes and are single-use (used flag set atomically)

#### OAuth Client Management (tag: `oauth-clients`)

Protected endpoints (require JWT + Casbin permission).

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/auth-admin/oauth/clients` | GET | Paginated client list (`?page=1&size=10&keyword=`) |
...
| `/auth-admin/oauth/clients/{id}/regenerate-secret` | POST | Regenerate client secret (returns plaintext once) |
...
```json
{
  ...
  "data": {
    "id": 2,
    "client_id": "client_abcdef1234567890",
    "client_secret": "hex_encoded_96_char_secret",
    "client_name": "My App",
    ...
  }
}
...
- `client_id` is auto-generated: `client_` + 16 hex chars
- `client_secret` is random 48-byte hex string, returned ONCE in plaintext
- `token_format`: `"jwt"` (stateless) or `"opaque"` (persisted in DB)
- `require_pkce`: `1` = must use PKCE, `0` = PKCE optional
...
OAuthClientEntity (auth_oauth_clients, registered via application_id)
    ├──1:N──> OAuthAuthorizationCodeEntity (temporary, 10-min TTL)
    ├──1:N──> OAuthAccessTokenEntity (opaque mode only)
    └──1:N──> OAuthRefreshTokenEntity
```

### OAuth Entities

**OAuthClientEntity** (`auth_oauth_clients`)
| Field | Type | Description |
|-------|------|-------------|
| id | i64 (auto) | Primary key |
| client_id | String | Public OAuth2 client identifier |
| client_secret_hash | String | bcrypt hashed client secret |
| application_id | i64 | FK to auth_applications.id |
| client_name | String | Display name |
| redirect_uris | String (JSON) | JSON array of allowed redirect URIs |
| grant_types | String (JSON) | JSON array: `["authorization_code","password","refresh_token"]` |
| scopes | String (JSON) | JSON array: `["openid","profile","read","write","admin"]` |
| token_format | String | `"jwt"` (stateless) or `"opaque"` (DB-persisted) |
| access_token_ttl | i32 | Access token lifetime in seconds |
| refresh_token_ttl | i32 | Refresh token lifetime in seconds |
| require_pkce | i8 | 1=must use PKCE, 0=optional |
| status | i8 | 1=enabled, 0=disabled |
| created_at | DateTime | Creation time |
| updated_at | DateTime | Last update time |

**OAuthAuthorizationCodeEntity** (`auth_oauth_authorization_codes`)
| Field | Type | Description |
|-------|------|-------------|
| id | i64 (auto) | Primary key |
| code | String | Authorization code (128-char hex, unique) |
| client_id | i64 | FK to auth_oauth_clients.id |
| user_id | Option\<i64\> | Authorizing user (NULL for client_credentials) |
| redirect_uri | String | Redirect URI used in the request |
| scopes | Option\<String\> | Granted scopes |
| code_challenge | Option\<String\> | PKCE code_challenge (S256) |
| code_challenge_method | Option\<String\> | PKCE method (e.g., "S256") |
| expires_at | DateTime | Expiry (10 minutes after creation) |
| used | i8 | 1=consumed (anti-replay) |
| created_at | DateTime | Creation time |

**OAuthAccessTokenEntity** (`auth_oauth_access_tokens`) — opaque mode only
| Field | Type | Description |
|-------|------|-------------|
| id | i64 (auto) | Primary key |
| token_hash | String | SHA256(access_token) |
| client_id | i64 | FK to auth_oauth_clients.id |
| user_id | Option\<i64\> | Token owner |
| scopes | Option\<String\> | Granted scopes |
| expires_at | DateTime | Expiry time |
| revoked | i8 | 1=revoked |
| created_at | DateTime | Creation time |

**OAuthRefreshTokenEntity** (`auth_oauth_refresh_tokens`)
| Field | Type | Description |
|-------|------|-------------|
| id | i64 (auto) | Primary key |
| token_hash | String | SHA256(refresh_token) |
| client_id | i64 | FK to auth_oauth_clients.id |
| user_id | i64 | Token owner |
| scopes | Option\<String\> | Granted scopes |
| access_token_id | Option\<i64\> | FK to auth_oauth_access_tokens (NULL for JWT mode) |
| expires_at | DateTime | Expiry time |
| revoked | i8 | 1=revoked (rotation) |
| created_at | DateTime | Creation time |
...
| V13 | `V13__create_auth_oauth_tables.sql` | OAuth 2.0 tables (clients, auth_codes, tokens) |
...
- [oauth_handler.rs](file:///d:/tdcare/genies/crates/auth-admin/src/interfaces/handler/oauth_handler.rs) — OAuth 2.0 HTTP endpoints
- [oauth_client_handler.rs](file:///d:/tdcare/genies/crates/auth-admin/src/interfaces/handler/oauth_client_handler.rs) — OAuth client management CRUD
- [oauth_token_service.rs](file:///d:/tdcare/genies/crates/auth-admin/src/application/oauth_token_service.rs) — OAuth token service (authorize/token/introspect/revoke)
- [oauth_client_service.rs](file:///d:/tdcare/genies/crates/auth-admin/src/application/oauth_client_service.rs) — OAuth client application service
- [oauth_dto.rs](file:///d:/tdcare/genies/crates/auth-admin/src/application/oauth_dto.rs) — OAuth DTOs (authorize/token/introspect requests & responses)
- [oauth_domain_service.rs](file:///d:/tdcare/genies/crates/auth-admin/src/domain/service/oauth_domain_service.rs) — OAuth domain service (clients, auth codes, tokens)
- [oauth_client_repository.rs](file:///d:/tdcare/genies/crates/auth-admin/src/domain/repository/oauth_client_repository.rs) — OAuth client queries
- [oauth_client_entity.rs](file:///d:/tdcare/genies/crates/auth-admin/src/domain/entity/oauth_client_entity.rs) — OAuthClientEntity
- [oauth_authorization_code_entity.rs](file:///d:/tdcare/genies/crates/auth-admin/src/domain/entity/oauth_authorization_code_entity.rs) — OAuthAuthorizationCodeEntity
...
- [V13__create_auth_oauth_tables.sql](file:///d:/tdcare/genies/crates/auth-admin/migrations/V13__create_auth_oauth_tables.sql) — OAuth 2.0 tables migration

## Known Pitfalls

### 1. RBatis "decode empty array value" Error

**Symptom:** `{"code":"-1","msg":"decode empty array value"}` when querying via `py_sql` methods that return `Option<T>`.

**Root cause:** RBatis can't deserialize an empty MySQL result set `[]` into `Option<T>` (it expects `null` for `None` or `{...}` for `Some`).

**Affected pattern:**
```rust
#[py_sql("SELECT * FROM table WHERE id = #{id}")]
pub async fn find_by_id(rb: &dyn Executor, id: &i64) -> rbatis::Result<Option<Entity>> {
    impled!()
}
// ❌ Fails with "decode empty array value" when no rows match
```

**Fix:** Change return type to `Vec<T>` and use `.into_iter().next()` at call sites:
```rust
#[py_sql("SELECT * FROM table WHERE id = #{id}")]
pub async fn find_by_id(rb: &dyn Executor, id: &i64) -> rbatis::Result<Vec<Entity>> {
    impled!()
}
// Call site: .await?.into_iter().next().ok_or("not found")?
```

This pattern is already used by `select_by_instance_id` in [app_instance_repository.rs](file:///d:/tdcare/genies/crates/auth-admin/src/domain/repository/app_instance_repository.rs).

### 2. expire_at() Timezone Mismatch

**Symptom:** Authorization codes always appear expired ("授权码已过期") even right after creation.

**Root cause:** `chrono::Utc::now()` produces UTC time, but `rbdc::DateTime::now()` produces local time (UTC+8). If `expire_at()` uses `chrono::Utc::now()` and the comparison uses `rbdc::DateTime::now()`, there is an 8-hour offset causing `expires_at < now` to always be true.

**Fix:** Always use `chrono::Local::now()` in `expire_at()` to align with `rbdc::DateTime::now()`.

```rust
// ❌ Wrong
fn expire_at(ttl_secs: i64) -> rbdc::DateTime {
    let t = chrono::Utc::now() + chrono::Duration::seconds(ttl_secs);
    // ...
}
...
### 3. PKCE with empty code_challenge

**Symptom:** Token exchange fails with "缺少 code_verifier (PKCE)" even when PKCE was not used during authorization.

**Root cause:** RBatis stores `Option::None` as an empty string `""` instead of `NULL` in VARCHAR columns, causing `code_entity.code_challenge` to be `Some("")` instead of `None`. The PKCE check `if let Some(ref challenge)` then triggers for empty strings.

**Fix:** Skip PKCE verification when challenge is empty:
```rust
...
### 4. OAuth Token Endpoint Content-Type

**Symptom:** Token endpoint returns 400 with field extraction errors.

**Root cause:** The token endpoint expects `Content-Type: application/x-www-form-urlencoded`, NOT `application/json`. Sending JSON body will fail to parse the grant_type, code, and other fields.

**Correct usage:**
```bash
curl -X POST ".../oauth/token" \
  -H "Content-Type: application/x-www-form-urlencoded" \
  -d "grant_type=authorization_code&code=xxx&client_id=xxx&redirect_uri=xxx"
```

### 5. OAuth Authorize Endpoint Reads User from Authorization Header

**Symptom:** Authorization endpoint redirects to login page even though the user already logged in via browser cookies.

**Root cause:** The `/oauth/authorize` endpoint reads the user's JWT from the `Authorization: Bearer <token>` header, NOT from cookies. Requests without this header will be redirected to the login page.
...
### 6. Seed Data bcrypt Hash Mismatch

**Symptom:** `refresh_token` or `client_credentials` grants fail with "client_secret 不正确" even with the correct secret.

**Root cause:** The seed data in `V13__create_auth_oauth_tables.sql` contains a bcrypt hash for `admin-ui` that may not match `"admin-ui-secret"`. The hash `$2b$12$LJ3m4ys3Gwo3GQ1GVsRIIO8FhSjDWxB1f1LMDvRQq0RNv9yXOZsxe` may have been generated with a different configuration.

**Fix:** Regenerate the client secret via the API:
```bash
curl -X POST ".../oauth/clients/1/regenerate-secret" \
  -H "Authorization: Bearer <ADMIN_JWT>"
```

### 7. Startup Directory MUST be crates/auth-admin

**CRITICAL:** auth-admin **MUST** be started from the `crates/auth-admin/` directory to load the correct `application.yml`. Starting from the project root will load the wrong config with different port, database name, and settings.
